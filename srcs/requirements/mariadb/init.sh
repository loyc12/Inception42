...
GRANT ALL PRIVILEGES ON "${MYSQL_DTBS_NAME}"'.*' TO "${MYSQL_USER_NAME}"@'%' IDENTIFIED BY "${MYSQL_USER_PSWD}" WITH GRANT OPTION;

GRANT ALL PRIVILEGES ON '*.*' TO 'root'@'localhost' IDENTIFIED BY "${MYSQL_ROOT_PSWD} WITH GRANT OPTION;";
